<?php
   ...
   imagerectangle( $img, 0, 0, 149, 49,  $rand2);
 
   while($y1!=50){
    imageline( $img, 0, $y1, 150, $y1, $rand2 );
    $y1=$y1+5;
   }
    while($x1!=150){
   ...
   $i=0;
   while($i!=15){
    imageline( $img, rand(0, 150), rand(0, 50), rand(0, 150), rand(0, 50),  $rand );
    $i=$i+1;
   }
   $k=1.6;
   $im1=imagecreatetruecolor(150*$k,50*$k);
   imagecopyresized($im1, $img, 0, 0, 0, 0, 150*$k, 50*$k, 150, 50);
   $im2=imagecreatetruecolor(150,50); 
   imagecopyresampled($im2, $im1, 0, 0, 0, 0, 150, 50, 150*$k, 50*$k);
   imagepng($im2); 
   imagedestroy($im2);
?>
